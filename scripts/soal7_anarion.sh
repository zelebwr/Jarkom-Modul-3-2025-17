...
APP_KEY=
APP_DEBUG=true
APP_URL=http://anarion.k17.com:8003
...
cat > /etc/nginx/sites-available/anarion.k17.com << 'EOF'
server {
    # anarion listens on port 8003
    listen 8003;
...
ln -s /etc/nginx/sites-available/anarion.k17.com /etc/nginx/sites-enabled/
